...
use App\Models\RsiaBerkasIht;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class RsiaBerkasIhtController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        $data = RsiaBerkasIht::select(array_map('trim', explode(',', $select)))
    ...
    public function store(Request $request)
    {
        ...
        $nomor = \App\Helpers\komite\LastNomor::get(new RsiaBerkasIht(), $request->tgl_terbit);
        $buildedNomor = [
    ...
    public function show($base64NomorTglTerbit)
    {
        ...
        $data = RsiaBerkasIht::where('nomor', $identifier[0])
    ...
    public function update(Request $request, $base64NomorTglTerbit)
    {
        ...
        $data = RsiaBerkasIht::where('nomor', $request->nomor)
    ...
    public function destroy($base64NomorTglTerbit)
    {
        ...
        try {
            DB::transaction(function () use ($data) {
                $data->delete();
                \App\Helpers\Logger\RSIALogger::berkas("data deleted successfully", 'info', ['data' => $data->toArray()]);
            });
        } catch (\Exception $e) {
            \App\Helpers\Logger\RSIALogger::berkas("failed to delete data", 'error', ['data' => $data->toArray(), 'error' => $e->getMessage()]);
